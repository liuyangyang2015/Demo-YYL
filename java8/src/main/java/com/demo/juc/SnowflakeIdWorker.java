package com.demo.juc;//public class SnowflakeIdWorker {
//    /**
//     * 开始时间戳
//     */
...
//    /**
//     * 二进制中，负数采用其绝对值的值得补码得到 * long型的-1 的值就32个1
//     */
...
//
//    /**
//     * 初始化并配置机器码id和数据id * * @param workerId 0-31 * @param dataId   0-31
//     */
...
//
//    /**
//     * 生成主键id，理论上应该在调用了init方法之后，调用生成的方式是有效的 * 不然所有的id都默认是按照机器码和数据id都是0的情况处理 * * @return 8个字节的长整型
//     */
//    public synchronized long genNextId() {
//        long timeStamp = genTimeStamp(); /**表示系统的时间修改了*/if (timeStamp < this.lastTimeStamp) {
...
//        }
//        if (timeStamp == this.lastTimeStamp) { /**查看序列是否溢出*/this.sequence = (this.sequence + 1) & maxSequence;
//            if (this.sequence == 0) { /**当出现溢出的时候，阻塞到下一个毫秒*/timeStamp = this.toNextMillis(this.lastTimeStamp);
//            }
//        } else {  /**此时表示时间戳跟最后的时间戳不一致,需要重置序列*/this.sequence = 0L;
//        }
//        this.lastTimeStamp = timeStamp; //通过移位或运算拼接组成64ID号 return ((timeStamp - startTime) << timestampLeftShift) | (dataId << dataShiftBits) | (worderId << sequenceBits) | sequence; } /** * 生成当前时间戳，单独写一个方法的原因是，若之后的时候修改扩展，不影响之前的业务， * 只在这个方法里面处理我们需要的数据 * * @return */ private long genTimeStamp() { return System.currentTimeMillis(); } /** * 阻塞到下一个毫秒，直到获得新的时间戳 * * @param lastTimeStamp 上传生成id的时间戳 * @return */ private long toNextMillis(long lastTimeStamp) { long timeStamp = this.genTimeStamp(); while (timeStamp <=>
//
//
//    /**
//     * 使用静态内部类实现单例
